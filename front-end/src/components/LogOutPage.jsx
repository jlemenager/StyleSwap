        ...
        const checkIfRealAccount = async() => {
            for (let i = 0;i<response.data.users.length;i++){
                if (userInfo.password == response.data.users[i].password && userInfo.username == response.data.users[i].username){
                    const putRequest = await axios.put(`https://styleswap-production.up.railway.app/api/userinfo/logoutpage/${response.data.users[i]._id}`, {...userInfo, isLoggedIn: false})
